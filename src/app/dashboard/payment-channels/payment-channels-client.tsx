...
export function PaymentChannelsClient({ initialChannels }: PaymentChannelsClientProps) {
  ...
  const handleToggleActive = async (id: string, currentStatus: boolean) => {
    try {
      ...
      if (currentUser?.id) {
        await logError('payment_channel.toggle_failed', error, currentUser.id, { channel_id: id })
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (currentUser?.id) {
        await logError('payment_channel.delete_failed', error, currentUser.id, {
  ...
  return (
    <>
      ...
      {/* Dialog for Create/Edit — conditionally mounted */}
      {crudDialog.isOpen && (
      <Dialog open onOpenChange={(open) => !open && handleCloseDialog()}>
