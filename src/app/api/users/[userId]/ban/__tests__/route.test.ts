...
function makeRequest(userId: string) {
  return createMockRequest(`/api/users/${userId}/ban`, { method: 'POST' })
}
...
describe('POST /api/users/[userId]/ban', () => {
  ...
  it('should return 401 when not authenticated', async () => {
    ...
    const res = await POST(makeRequest(TARGET_USER_ID), { params: Promise.resolve({ userId: TARGET_USER_ID }) })
  ...
  it('should return 400 when trying to ban yourself', async () => {
    ...
    const res = await POST(makeRequest(AUTH_USER_ID), { params: Promise.resolve({ userId: AUTH_USER_ID }) })
