...
import { logActivity, logWarn, logError } from '@/lib/logger'
...
export default function PaymentChannelsPage() {
  ...
  useEffect(() => {
    const fetchChannels = async () => {
      try {
        const { data, error } = await (supabase.from('payment_channels') as any)
          .select('*')
        ...
        console.error('[PaymentChannels] Failed to load channels:', error)
        toast.error('Failed to load payment channels', { description: getUserFriendlyMessage(error) })
      } finally {
        setLoading(false)
      }
    }
